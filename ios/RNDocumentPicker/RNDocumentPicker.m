...
- (NSMutableDictionary *)getMetadataForUrl:(NSURL *)url error:(NSError **)error
{
    ...
    [coordinator coordinateReadingItemAtURL:url options:NSFileCoordinatorReadingResolvesSymbolicLink error:&fileError byAccessor:^(NSURL *newURL) {
        ...
        if (!fileError) {
            [result setValue:[[RNDocumentPicker copyToUniqueDestinationFrom: newURL] absoluteString] forKey:FIELD_URI];
...
+ (NSURL *)copyToUniqueDestinationFrom:(NSURL *) url
{
    ...
    NSURL *attemptedDestination = [NSURL fileURLWithPath:[documentsDir.path stringByAppendingPathComponent:[url lastPathComponent]]];
    NSFileManager *filemgr = [NSFileManager defaultManager];
    NSURL *destinationUrl = [RNDocumentPicker ensureUniqueDestinationFor:attemptedDestination];
...
+ (NSURL *)ensureUniqueDestinationFor:(NSURL *) url
{
    NSFileManager *filemgr = [NSFileManager defaultManager];
    NSURL *toBecomeUnique = url;
    int i = 1;
    
    while ([filemgr fileExistsAtPath:toBecomeUnique.path]) {
        NSString *pathWithoutFilename = [url.path stringByDeletingLastPathComponent];
        NSString *fnameWithoutExtension = [url.path.lastPathComponent stringByDeletingPathExtension];
        NSString *possiblyUniqueFilepath = [NSString stringWithFormat:@"%@/%@%d.%@", pathWithoutFilename, fnameWithoutExtension, i, url.path.pathExtension];
        toBecomeUnique = [NSURL fileURLWithPath:possiblyUniqueFilepath];
        
        i++;
    }
    return toBecomeUnique;
}
